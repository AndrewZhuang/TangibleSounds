...
class Stream_Analyzer:
    ...
    def __init__(self,
        ...
        self.frames = 0
        self.noise_cancel_dampening = np.zeros(n_frequency_bins)
        self.noise_cancel_factor = 0
        ...
        if self.visualize:
            ...
            # from src.ripplevisualizer import RippleVisualizer
        ...
        if self.visualize:
            ...
            # self.visualizer = RippleVisualizer(self)
    ...
    def update_features(self, n_bins = 3):
        ...
        self.strongest_frequency_ix = np.argmax(self.fft)
        # print(len(self.fft), len(self.fftx))
        self.strongest_frequency = self.fftx[self.strongest_frequency_ix]
        self.loudest = np.max(self.fft)
    ...
    def get_audio_features(self):
        ...
        if self.stream_reader.new_data:  #Check if the stream_reader has new audio data we need to process
            ...
            if self.frames < 300:
                # self.noise_cancel_factor = max(self.noise_cancel_factor, np.max(self.frequency_bin_energies))
                for i in range(self.n_frequency_bins):
                    self.noise_cancel_dampening[i]+=self.frequency_bin_energies[i]
            else:
                for i in range(self.n_frequency_bins):
                    self.frequency_bin_energies[i]-=self.noise_cancel_dampening[i]//300
                    # self.frequency_bin_energies[i]-=self.noise_cancel_factor
